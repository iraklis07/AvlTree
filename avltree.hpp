...
public: 
  bool sanity () const {
    int size = 0;
    if (root == nullptr && size == the_size) return true;
    return (root->parent == nullptr && 
            sanitynode(root, size) && 
            size == the_size);
  }
//we need post-order traversal
private:
  bool sanitynode (node *p, int &size) const {
    bool isavl = true;
    visit(p, size, isavl);
    return isavl;  
  }

  int visit (node *p, int  &size, bool &b) const {
    if (p != nullptr) {
      size++;
      int heightleft = visit(p->left, size, b) ;
      int heightright = visit(p->right, size, b);
      if (children_parent_consistency(p) && isBstree(p) &&
          isAvl(p, heightleft, heightright) && b)
        b = true;
      else b = false;
  ...
  bool children_parent_consistency (node *t) const {
    if ((t->left == nullptr || (t->left != nullptr && t->left->parent == t)) &&
        (t->right == nullptr || (t->right != nullptr && t->right->parent == t)))
      return true;
    else return false;
  }
  
  bool isBstree (node *t) const {
    if ((t->left == nullptr || (t->left != nullptr && t->left->data < t->data)) &&
        (t->right == nullptr || (t->right != nullptr && t->right->data > t->data)))
      return true;
    else return false;
  }

  bool isAvl (node *t, int heightl, int heightr) const {
    int diff = heightr-heightl;
    if ((diff > 1 || diff < -1) || t->balance != diff) return false;
    else return true; 
  }   
};
